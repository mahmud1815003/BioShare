...
import { useAddDoctorMutation } from '../../../redux/HospitalAdmi/hospitalAdminApi';
...
const DoctorForm = () => {
    ...
    const [addDoctor, { data: responseData, error, isSuccess }] = useAddDoctorMutation();
    ...
    if (hospital && !Error) {
        content = hospital?.map((data, idx) => {
            return <option className='bg-[#150050] text-white z-10' value={`${data.name}`}>{data.name}</option>
        })
    }
    return (
        <div className='bg-[#150050] w-[650px] mx-auto py-6 mt-6'>
            <form onSubmit={handleSubmit} className='flex flex-col items-center mx-auto px-6 py-4 rounded-md bg-[#150050]'>
                <h1 className='py-4 text-4xl font-bold bg-[#150050] text-white'>Add/Update a Doctor</h1>
                <div className='my-2 flex flex-col bg-[#150050]'>
                    ...
                    <div className='my-2 flex flex-col bg-[#150050] text-white z-10'>
                        ...
                        {error?.data?.posting && <p className='bg-[#150050] text-red-600 w-full '>{error?.data?.posting?.msg}</p>}
                    </div>
                    <div className='bg-[#150050]'>
